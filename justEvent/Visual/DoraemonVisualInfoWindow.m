    ...
    if (@available(iOS 13.0, *)) {
        if ([self.traitCollection hasDifferentColorAppearanceComparedToTraitCollection:previousTraitCollection]) {
            if (UITraitCollection.currentTraitCollection.userInterfaceStyle == UIUserInterfaceStyleDark) {
                [self.closeBtn setImage:[UIImage imageNamed:@"doraemon_close_dark"] forState:UIControlStateNormal];
            } else {
                [self.closeBtn setImage:[UIImage imageNamed:@"doraemon_close"] forState:UIControlStateNormal];
            }
        }
    }
}
...
- (void)initUI {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        UIImage *closeImage = [UIImage imageNamed:@"doraemon_close"];
        if (@available(iOS 13.0, *)) { 
            if (UITraitCollection.currentTraitCollection.userInterfaceStyle == UIUserInterfaceStyleDark) {
                closeImage = [UIImage imageNamed:@"doraemon_close_dark"];
            }
        }
        ...
        self.infoLabel.textColor = [UIColor colorWithHexString:@"333333"];
...
- (void)commonInit {
    ...
    self.layer.borderColor = [[UIColor colorWithHexString:@"999999"] colorWithAlphaComponent:0.2].CGColor;
